    ...
    reducers: {
        addToCart : (state, action) => {
            ...
            if(cartItem) {
                const updatedCart = state.cartItems.map((item) => 
                    item.product._id === productId ? { ...item, quantify:cartItem.quantify + 1} : item
                );
                state.cartItems = updatedCart;
            } else {
                state.cartItems.push({product, quantify: 1})
            }
        },
        removeFromCart : (state, action) => {
            ...
            if(cartItem.quantify === 1){
                ...
                updatedCart = state.cartItems?.map((item) => item.product._id === productId ? {...item, quantify: item.quantify - 1 }
                : item 
                );
            }
            state.cartItems = updatedCart;
